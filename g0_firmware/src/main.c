...
/* Per-port command line buffer/state. Each UART port (console, USART1)
...
/* ====================================================================
   ...
   Services BOTH UART ports (console + USART1) every tick. Each port
    ...
    while (UART_GetChar(port, &uart_char)) {
        if (uart_char == '\r' || uart_char == '\n') {
            ...
            /* Printable character - add to buffer and echo back on
               ...
               only ever reach the console, never USART1) */
...
static void uart_poll_thread(void *arg1, void *arg2, void *arg3)
{
    ...
    while (1) {
        ...
        service_uart_port(UART_PORT_USART1);
