 ...
 *
 *   UART_PORT_CONSOLE -> ST-Link VCOM console (USART2, PA2 TX / PA3 RX, 115200 8N1)
 *   UART_PORT_USART1  -> device-to-device UART link (USART1, PC4 TX / PC5 RX, 115200 8N1)
 *
...
void UART_Init(void)
{
    ...
    init_one_port(UART_PORT_USART1,
                   DEVICE_DT_GET(DT_NODELABEL(usart1)),
                   "usart1");
#else
    printk("USART1 devicetree node not found - is it enabled in your overlay?\n");
    s_ports[UART_PORT_USART1].dev = NULL;
#endif
}
